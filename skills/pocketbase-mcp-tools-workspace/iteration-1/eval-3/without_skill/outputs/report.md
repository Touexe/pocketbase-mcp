...
The PocketBase instance has **28 collections**. Broken down by type:

### View collections (5) — these are views, not base/auth

| # | Collection | ID | Fields |
|---|------------|----|--------|
| 1 | `team_challenges_flags` | pbc_3302673531 | 2 |
| 2 | `_players_attempts` | pbc_826859169 | 6 |
| 3 | `_team_players_stats` | pbc_4040147337 | 7 |
| 4 | `_dashboard_metrics` | pbc_3864132092 | 7 |
| 5 | `_submission_stats` | pbc_3757750218 | 7 |
...
## (b) MCP tool calls made

1. `mcp__pocketbase__connect(as_="status")`
   - Response summary: Authenticated as `superuser:mrrtou@protonmail.com`; no startup auth error.

2. `mcp__pocketbase__describe_schema()`
   - Response summary: Returned all 28 collections with name/id/type/field_count. Types present: 21 `base`, 2 `auth` (`_superusers`, `admins`), 5 `view` (`team_challenges_flags`, `_players_attempts`, `_team_players_stats`, `_dashboard_metrics`, `_submission_stats`).
